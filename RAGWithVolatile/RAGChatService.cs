...
internal sealed class RAGChatService(
    ...
    VectorStoreTextSearch<TextParagraph> vectorStoreTextSearch,
    Kernel kernel,
    [FromKeyedServices("AppShutdown")] CancellationTokenSource appShutdownCancellationTokenSource)
{
    ...
    {
        var pdfFiles = string.Empty; //= string.Join(", ", ragConfigOptions.Value.PdfFilePaths ?? []);
    ...
    private async Task LoadDataAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            foreach (var pdfFilePath in PdfFilePaths)
            {
                ...
                await dataLoader.LoadPdf(pdfFilePath, 2, 100, cancellationToken);
            }
        }
        catch (Exception ex)
        {
