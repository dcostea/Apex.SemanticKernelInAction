...
/// Using semantic functions (prompts) exposed by MCP Server
Console.WriteLine("\nAVAILABLE PROMPTS:");
var mcpPrompts = await client.ListPromptsAsync();
foreach (var mcpPrompt in mcpPrompts)
{
    Console.WriteLine($"- {mcpPrompt.Name}: {mcpPrompt.Description}");
}
...
var prompt = await client.GetPromptAsync(promptName, arguments);
var promptMessages = prompt.ToChatMessages();
