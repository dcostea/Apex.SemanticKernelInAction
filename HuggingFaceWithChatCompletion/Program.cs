...
builder.AddHuggingFaceChatCompletion(
    model: Model,
    endpoint: new Uri($"{ModelHub}{Model}"),
    apiKey: configuration["HuggingFace:ApiKey"]!)
.Build();
//builder.Services.AddLogging(c => c.AddConsole().SetMinimumLevel(LogLevel.Trace));
Kernel kernel = builder.Build();

KernelPlugin motorsPlugin = kernel.ImportPluginFromType<MotorsPlugin>();
...
HuggingFacePromptExecutionSettings executionSettings = new()
{
    ...
    TopP = 0.95F,
    MaxTokens = 2000,
    FunctionChoiceBehavior = FunctionChoiceBehavior.Auto(motorsPlugin)
};
...
var systemPrompt = """
    ...
    YOU MUST ONLY USE THE PROVIDED FUNCTIONS (MotorsPlugin.forward, MotorsPlugin.backward, MotorsPlugin.turn_left, MotorsPlugin.turn_right, MotorsPlugin.stop) with their arguments.
...
if (!chatHistory.Any(m => m.Role == AuthorRole.Tool))
{
    ...
    await FunctionCallingFix(kernel, motorsPlugin, response);
}
