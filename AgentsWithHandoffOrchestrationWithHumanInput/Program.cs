...
ChatCompletionAgent safetyAgent = new()
{
    ...
    Instructions = """
        ...
        4. If the `safety clearance` is denied, ask the USER for overriding the safety clearance.
        5. ALWAYS save the `safety clearance` using tool ```save_safety_clearance``` with argument `safety clearance`
        6. IF clearance is granted THEN hand off to MotorsAgent
